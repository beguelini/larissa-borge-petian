...
import { consultationPriceCents, ebookConsultationDiscountedPriceCents, formatConsultationPrice } from '../lib/consultation-pricing'
...
export function ConsultationSalesScreen() {
  ...
  const [ebookConsultationBenefit, setEbookConsultationBenefit] = useState(false)
  const result = useMemo(() => completedResult ?? scoreQuiz(answers), [answers, completedResult])

  useEffect(() => { window.scrollTo({ top: 0, behavior: 'smooth' }) }, [phase])

  function begin() { setAnswers({}); answersRef.current = {}; setQuestionIndex(0); setCompletedResult(null); setPhase('intake'); setError('') }
  ...
  function answer(questionId: string, optionId: string) { const nextAnswers = { ...answersRef.current, [questionId]: optionId }; answersRef.current = nextAnswers; setAnswers(nextAnswers) }
  function advance() { if (questionIndex === quizQuestions.length - 1) void savePatient(answersRef.current); else setQuestionIndex((current) => current + 1) }
  async function savePatient(currentAnswers: QuizAnswers) {
    ...
    try {
      ...
      const saved = await response.json() as { ebookConsultationBenefit?: boolean }
      setEbookConsultationBenefit(saved.ebookConsultationBenefit === true)
