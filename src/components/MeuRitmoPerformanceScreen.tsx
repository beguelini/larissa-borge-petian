...
export function MeuRitmoPerformanceScreen() {
  ...
  const chart = useMemo(() => result.registrationsByDay.map(({ date, count }, index, days) => ({
    date,
    count,
    height: Math.max(count ? 8 : 2, Math.round((count / maxDaily) * 100)),
    showDate: days.length <= 10 || index === 0 || index === days.length - 1 || index % Math.max(2, Math.ceil((days.length - 1) / 7)) === 0,
  })), [result.registrationsByDay, maxDaily])
